...
RUN apt-get update && apt-get install -y texlive-latex-base texlive-fonts-recommended texlive-fonts-extra texlive-latex-extra texlive-lang-spanish
...
RUN Rscript -e 'remotes::install_version("RSQLite",upgrade="never", version = "2.2.8")'
...
EXPOSE 3838
CMD  ["R", "-e", "options('shiny.port'=3838,shiny.host='0.0.0.0');VisualizadorFallasET::run_app()"]
